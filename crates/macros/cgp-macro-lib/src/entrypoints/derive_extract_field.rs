use cgp_macro_core::types::cgp_data::{
    ...
    derive_has_extractor_ref_impl, derive_partial_data_impl_from_enum,
};
...
pub fn derive_extract_field_from_enum(context_enum: &ItemEnum) -> syn::Result<TokenStream> {
    ...
    let partial_data_impl =
        derive_partial_data_impl_from_enum(context_enum, &extractor_ident, false)?;
    let partial_ref_data_impl =
        derive_partial_data_impl_from_enum(context_enum, &extractor_ref_ident, true)?;
