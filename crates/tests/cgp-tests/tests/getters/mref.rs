//! `#[cgp_getter]` returning `MRef<'_, String>` (an owned-or-borrowed value):
//! the getter reads a `String` field and wraps the borrow in `MRef::Ref`. The
//! context binds the source field via `UseField`.
//!
