//! A namespace whose path segment is a type (a struct), not a `Symbol!` string.
//!
//! When a `cgp_namespace!` / `#[prefix]` path segment is a PascalCase name like
//! `MyApp` that resolves to a type, the macro places the type directly into the
//! `PathCons<..>` path (contrast `namespace_symbol_path`, where `my_app` becomes a
//! type-level `Symbol<..>` string). The `cgp_namespace!`, `#[prefix]`-component,
