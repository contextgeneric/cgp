//! An inheriting namespace that rewrites a path prefix.
//!
//! `cgp_namespace! { new ExtendedNamespace: DefaultNamespace { @cgp.core.error => @app } }`
//! inherits every `DefaultNamespace` entry and additionally rewrites the
//! `cgp.core.error` path prefix onto `app`, so a context using this namespace can
//! wire the error components under a shorter `@app.*` path. This snapshot captures
