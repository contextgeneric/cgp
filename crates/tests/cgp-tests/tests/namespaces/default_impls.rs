//! Reusable namespace + per-type default impls used by `default_impls_wiring`.
//!
//! Defines the `Show`/`ShowImpl` component attached to `DefaultNamespace` via
//! `#[prefix(@test in DefaultNamespace)]`, provider structs registered as per-type
//! defaults with `#[default_impl(... in DefaultImpls1<...>)]`, a `DefaultShowComponents`
//! namespace table built with `cgp_namespace!`, and an inheriting `ExtendedNamespace`
//! (`cgp_namespace! { new ExtendedNamespace: DefaultNamespace { .. } }`). These
