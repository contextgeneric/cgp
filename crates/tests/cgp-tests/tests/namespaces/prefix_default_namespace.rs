//! Attaching a component to `DefaultNamespace` and wiring a context by full path.
//!
//! `#[prefix(@app.MyComponents.FooProviderComponent in DefaultNamespace)]` gives
//! `Foo` a nested default path, so its snapshot carries a
//! `DefaultNamespace<..> for FooProviderComponent` impl targeting that
//! `RedirectLookup` path. A context then opens `namespace DefaultNamespace;` and
//! wires the CGP-shipped error components by their full dotted paths
//! (`@cgp.core.error.ErrorTypeProviderComponent`, `.ErrorRaiserComponent.String`).
