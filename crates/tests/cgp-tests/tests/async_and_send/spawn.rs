//! Spawning an async CGP future onto an executor that requires
//! `Future: Send + 'static`, without annotating `Send` throughout the abstract
//! code.
//!
//! The abstract types (`HasFooType`, `HasBarType`) and the async interfaces
//! (`CanFetchFoo`, `CanFetchBar`, `CanRunFooBar`) carry no explicit `Send`
//! bounds. A future built from them can still be handed to a `tokio::spawn`-like
//! function (here `dummy_spawn`, which demands `Future: Send + 'static`) by
//! implementing a proxy `SendRunner` on the *concrete* context: once the context
...
// An example App context that has Send-safe implementations
