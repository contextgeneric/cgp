//! `#[derive(CgpData)]` on a *generic* enum: the derive lifts the enum's type
//! parameters onto every generated impl and onto the `__Partial*` extractor
//! enums, so a variant's payload type is the corresponding type parameter rather
//! than a concrete type. Structural casts still line variants up by name, so a
//! two-variant instantiation upcasts into a three-variant one and downcasts back.
//!
...
// A two-variant instantiation, used as the cast counterpart. Plain derive.
