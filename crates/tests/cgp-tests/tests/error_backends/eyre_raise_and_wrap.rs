...
fn test_eyre_raise_and_wrap() {
    ...
    // The crate leaves eyre's `track-caller` feature off, since the recorded location would be a
    // line inside the backend, so the default handler prints no `Location:` section.
    assert_eq!(
        format!("{error:?}"),
        "while starting\n\nCaused by:\n   0: while loading\n   1: no file"
    );
