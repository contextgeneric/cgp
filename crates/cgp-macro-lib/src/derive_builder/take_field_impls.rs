...
pub fn derive_take_field_impls(
    ...
    for (current_index, current_field) in context_struct.fields.iter().enumerate() {
        ...
        for (other_index, other_field) in context_struct.fields.iter().enumerate() {
            ...
            if other_index != current_index {
                ...
                output_generic_args.push(parse2(quote! { IsNothing })?);
        ...
        let item_impl = parse2(quote! {
            impl #impl_generics TakeField< #tag_type >
                for #source_type
            #where_clause
            {
                ...
                fn take_field(self, _tag: ::core::marker::PhantomData< #tag_type >) -> (Self::Value, Self::Remainder) {
                    let value = self. #member;

                    let remainder = #builder_ident {
                        #builder_fields
                    };
