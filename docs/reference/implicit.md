...
This is why `#[implicit]` is the recommended starting point for basic CGP. It lets a newcomer write providers in [`#[cgp_fn]`](cgp_fn.md) and [`#[cgp_impl]`](cgp_impl.md) using only familiar function syntax, deferring the `HasField` machinery until they actually need to understand it.
...
`#[implicit]` is usable wherever CGP rewrites function bodies into providers: inside [`#[cgp_fn]`](cgp_fn.md) and inside the methods of a [`#[cgp_impl]`](cgp_impl.md) block. It is not a standalone macro — it is only meaningful as an argument attribute consumed by those macros.
...
The access expression depends on the argument type, following the same rules as [`#[cgp_auto_getter]`](cgp_auto_getter.md). For an owned type such as `f64` or `String`, the macro reads the field by reference and appends `.clone()`, so the body receives an owned value. The one special case worth knowing is `&str`: an argument typed `&str` is backed by a `String` field, and the access uses `.as_str()` rather than `.clone()`. Concretely:
...
Inside a [`#[cgp_impl]`](cgp_impl.md) block the rewrite is identical — the same `HasField` bounds are added to the impl's `where` clause and the same `let` bindings are prepended to the method body. For example:
...
A complete `#[cgp_fn]` capability with implicit arguments needs only a context that derives [`HasField`](derive_has_field.md) and contains the named fields:
...
`#[implicit]` is most often used inside [`#[cgp_fn]`](cgp_fn.md), which turns a function into a single-implementation capability, and inside [`#[cgp_impl]`](cgp_impl.md), which writes a provider for an existing component. It relies on [`#[derive(HasField)]`](derive_has_field.md) on the context to supply the field accessors that the generated bounds require. Its access rules — `.clone()` for owned values, `.as_str()` for `&str` — are shared with [`#[cgp_auto_getter]`](cgp_auto_getter.md), which is the better tool when the same fields are accessed across many methods or in the middle of a body. To bring in other CGP capabilities alongside implicit arguments, combine `#[implicit]` with [`#[uses]`](uses.md).
