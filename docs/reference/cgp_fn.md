...
Several companion attributes refine the generated code and are documented separately. [`#[uses(...)]`](uses.md) adds trait bounds on `Self` as impl-side dependencies; [`#[use_type(...)]`](use_type.md) imports an abstract type and rewrites its occurrences to fully-qualified form; [`#[use_provider(...)]`](use_provider.md) supports higher-order providers; [`#[extend(...)]`](extend.md) adds supertrait bounds to the generated trait; and [`#[extend_where(...)]`](extend_where.md) adds `where` predicates to the generated trait definition.
