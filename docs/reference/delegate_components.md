...
A value may itself open a nested table, which is the common shape for dispatching a generic-parameter component. Writing `UseDelegate<new InnerComponents { ... }>` as a value both wires the outer key to [`UseDelegate`](use_delegate.md) over an inner table and defines that inner table in place:
