...
The second extra construct is a `WithProvider` impl, which adapts the foundational [`HasType`/`TypeProvider`](../traits/has_type.md) machinery into this component. It implements the provider trait for `WithProvider<__Provider__>` whenever `__Provider__` is a `TypeProvider` for the component:
