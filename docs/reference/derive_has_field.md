...
Higher-level constructs are built directly on these generated impls. [`#[cgp_auto_getter]`](cgp_auto_getter.md) and [`#[cgp_getter]`](cgp_getter.md) (through the [`UseField`](use_field.md) provider) generate blanket impls whose `where` clauses are `HasField` bounds, and the [`#[implicit]`](implicit.md) argument form desugars function parameters into `get_field` calls. All of them assume the context has derived `HasField`; this derive is what makes them work.
...
It applies to structs with named fields and to structs with unnamed (tuple) fields. The two cases differ only in how each field's tag is computed: a named field is keyed by [`Symbol!("field_name")`](symbol.md), the type-level string of its identifier, while a tuple field is keyed by `Index<N>`, the type-level natural number of its position. Unit structs produce no impls because they have no fields.

The derive concerns itself only with the *field-level* view. To obtain the whole-struct view as a single type-level [`Product`](product.md), derive [`HasFields`](derive_has_fields.md) instead; the two are complementary and frequently derived together.
