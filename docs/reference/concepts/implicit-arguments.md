...
An implicit argument is a function parameter carrying the [`#[implicit]`](../attributes/implicit.md) attribute, whose name doubles as the context field name it reads from. The attribute is meaningful inside the two macros that rewrite function bodies into providers: [`#[cgp_fn]`](../macros/cgp_fn.md), which turns a free function into a single-implementation capability, and [`#[cgp_impl]`](../macros/cgp_impl.md), which writes a provider for an existing component. In both, the marked argument disappears from the signature and is fetched from the context instead.
...
The desugaring is the impl-side dependency pattern applied to values: each implicit argument becomes a [`HasField`](../traits/has_field.md) bound on the generated impl and a `let` binding that reads the field at the top of the body. The function above generates a `RectangleArea` trait whose method takes no arguments, and a blanket impl requiring `HasField<Symbol!("width"), Value = f64>` and the matching `height` bound, with `width` and `height` read from the context before the body runs. The same rewrite happens identically inside a [`#[cgp_impl]`](../macros/cgp_impl.md) method, where the bounds join the provider impl's `where` clause. The field bounds are satisfied by any context that derives [`#[derive(HasField)]`](../derives/derive_has_field.md) and carries fields of the matching names — no wiring is involved for `#[cgp_fn]`, since its blanket impl applies to every context that has the fields.

Because the argument name is the field name, the values are addressed by name through the type-level symbol [`HasField`](../traits/has_field.md) uses internally, and the whole `Symbol!`/`get_field`/`PhantomData` apparatus stays out of sight. What the author sees is a function with arguments; what the compiler sees is a provider with injected dependencies.
...
The [`#[implicit]`](../attributes/implicit.md) attribute is the construct itself, usable inside [`#[cgp_fn]`](../macros/cgp_fn.md) and [`#[cgp_impl]`](../macros/cgp_impl.md) — the former producing a single-implementation capability with no wiring, the latter a provider for a [`#[cgp_component]`](../macros/cgp_component.md). All of them desugar implicit arguments into [`HasField`](../traits/has_field.md) bounds, which a context supplies by deriving [`#[derive(HasField)]`](../derives/derive_has_field.md).

For repeated or mid-body field access, [`#[cgp_auto_getter]`](../macros/cgp_auto_getter.md) is the getter-trait counterpart that shares the same `.clone()`/`.as_str()` access rules. The whole model is value-level [impl-side dependencies](impl-side-dependencies.md): an implicit argument is a context dependency injected through a `where`-clause `HasField` bound and dressed as a function parameter.
