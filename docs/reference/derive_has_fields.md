...
`#[derive(HasFields)]` is the derive macro that gives a type a whole-shape view: it implements `HasFields` (and `HasFieldsRef`, `ToFields`, `FromFields`, `ToFieldsRef`) so that the entire struct or enum is described by a single type-level [`Product`](product.md) of named fields, or for an enum a type-level [`Sum`](sum.md) of named variants.
...
each field becomes a [`Field<Tag, Value>`](derive_has_field.md) entry — the `Value` wrapped together with its type-level name tag — and the entries are chained into a [`Product!`](product.md). The `HasFields` impl names that product, and `HasFieldsRef` names the same product with each value borrowed:
...
An enum expands to a [`Sum`](sum.md) instead of a product. Each variant becomes an [`Either`](sum.md) arm tagged by the variant name with [`Symbol!`](symbol.md), carrying that variant's own fields as a nested product, and the chain is terminated by `Void`. Starting from:
...
`#[derive(HasFields)]` is the structural counterpart to [`#[derive(HasField)]`](derive_has_field.md): `HasField` gives indexed, single-field access for dependency injection, while `HasFields` gives the aggregate view of the whole type. The `Fields` type it produces is built from [`Product`](product.md) for structs and [`Sum`](sum.md) for enums, with named entries tagged by [`Symbol!`](symbol.md). [`#[derive(CgpData)]`](derive_cgp_data.md) builds on this derive, generating `HasFields` together with the additional builder, partial-record, and field-update machinery needed for extensible data.
