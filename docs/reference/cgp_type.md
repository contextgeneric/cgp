...
`#[cgp_type]` defines an abstract-type component — a trait carrying a single associated type — by extending [`#[cgp_component]`](cgp_component.md) and generating the extra constructs that let a context choose the concrete type through wiring, most notably a [`UseType`](use_type.md) blanket impl.
...
A note on version history is warranted because older code and documentation differ here. As of v0.6.2, the default provider name changed from `ProvideType` to `TypeProvider` (so the suffix is now `TypeProvider`, and the foundational built-in provider trait is `TypeProvider`), and the convenience `{Type}Of` type alias that earlier versions generated was removed. Code written against pre-0.6.2 CGP that refers to `Provide{Type}` providers or `{Type}Of` aliases must be updated.
...
The first extra construct is the [`UseType`](use_type.md) blanket impl, which is the heart of `#[cgp_type]`. It implements the provider trait for `UseType<Scalar>` by setting the abstract associated type to the generic parameter `Scalar`. Starting from:
...
The second extra construct is a `WithProvider` impl, which adapts the foundational [`HasType`/`TypeProvider`](has_type.md) machinery into this component. It implements the provider trait for `WithProvider<__Provider__>` whenever `__Provider__` is a `TypeProvider` for the component:
...
`#[cgp_type]` is the abstract-type specialization of [`#[cgp_component]`](cgp_component.md), inheriting its full expansion and provider-name override syntax while keying the default name off the associated type. Its central generated construct is the [`UseType`](use_type.md) provider, the type-level analogue of the [`UseField`](use_field.md) provider that [`#[cgp_getter]`](cgp_getter.md) generates. It builds on CGP's foundational [`HasType`/`TypeProvider`](has_type.md) component via the generated `WithProvider` impl. Abstract-type components are wired with [`delegate_components!`](delegate_components.md) and checked with [`check_components!`](check_components.md), and they are imported into other definitions with [`#[use_type]`](use_type.md). When an abstract type's only role is to be a getter's return type, [`#[cgp_auto_getter]`](cgp_auto_getter.md) can declare it inline instead.

## Source

The macro entry point is `cgp_type` in [crates/macros/cgp-macro-lib/src/cgp_type.rs](../../crates/macros/cgp-macro-lib/src/cgp_type.rs), which extracts the single associated type, derives the default `{Type}TypeProvider` provider name from the associated type's identifier, runs the `#[cgp_component]` `preprocess → eval` pipeline, and converts the result into `ItemCgpType`. The logic lives in [crates/macros/cgp-macro-core/src/types/cgp_type/item.rs](../../crates/macros/cgp-macro-core/src/types/cgp_type/item.rs), which validates the trait shape (`extract_item_type_from_trait`) and builds the `UseType` and `WithProvider` provider impls. The runtime `HasType`, `TypeProvider`, and `UseType` definitions are in [crates/core/cgp-type/src/](../../crates/core/cgp-type/src/) (`traits/has_type.rs` and `impls/use_type.rs`). The `ProvideType` → `TypeProvider` rename and the removal of the `{Type}Of` alias are recorded under v0.6.2 in [CHANGELOG.md](../../CHANGELOG.md). Behavioral and expansion-snapshot tests are in [crates/tests/cgp-tests/tests/component_tests/abstract_types/](../../crates/tests/cgp-tests/tests/component_tests/abstract_types/) (notably `basic.rs`).
