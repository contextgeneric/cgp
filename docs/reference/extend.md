...
`#[extend(...)]` is accepted in [`#[cgp_fn]`](cgp_fn.md) and in [`#[cgp_component]`](cgp_component.md). It is not available in [`#[cgp_impl]`](cgp_impl.md), because a provider impl has no trait definition of its own to attach supertraits to — the supertraits belong to the component's trait, defined by `#[cgp_component]`.
...
In [`#[cgp_component]`](cgp_component.md) the effect is purely on the consumer trait, and it is exactly equivalent to writing the supertrait directly. The definition
