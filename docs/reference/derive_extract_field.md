...
Each single-payload variant's name becomes a type-level string [`Symbol!`](../../.claude/skills/cgp/SKILL.md) used as the variant's `Tag`, and its payload type becomes its value type. Generic parameters on the enum are carried onto the generated impls. The derive emits the same extractor impls that the variant path of [`#[derive(CgpData)]`](derive_cgp_data.md) emits — it is that slice in isolation, with no `HasFields` representation traits and no [`FromVariant`](derive_from_variant.md) constructors.
...
it first emits the owned partial enum `__PartialShape` and the borrowed partial enum `__PartialRefShape`. Each variant's payload is wrapped in a [`MapType`](../../.claude/skills/cgp/SKILL.md) marker, where `IsPresent` keeps the payload and `IsVoid` maps it to the empty [`Void`](sum.md) type; the borrowed form adds a `MapTypeRef` parameter that selects shared or mutable references:
