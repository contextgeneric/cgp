...
Several companion attributes are supported on a `#[cgp_impl]` block and are processed before the provider-trait rewrite. Method parameters marked [`#[implicit]`](implicit.md) are extracted from the signature and turned into `HasField` reads on the context. [`#[uses(...)]`](uses.md) adds simple trait bounds on `Self`, [`#[use_type(Trait::Type)]`](use_type.md) imports an abstract type and rewrites its occurrences to fully qualified form, and [`#[use_provider(...)]`](use_provider.md) supports higher-order providers by adding the `Self` parameter to an inner provider bound and by dispatching method calls to a named provider.
