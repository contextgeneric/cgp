...
A generated item's structural tokens — the `impl` keyword, the trait reference, the self type — carry whatever span the macro stamps on them, and by default `parse_internal!`/`quote!` stamp the macro's `call_site` span, which covers the whole invocation. The visible symptom is a compiler error on a generated item's *header* — a coherence conflict (`E0119`) between two generated impls, an unsatisfied bound, a name-resolution failure — underlining the entire macro block instead of the entry, attribute, or impl the user actually wrote. The fix is to re-span each generated item onto its originating token, and the codebase does this three ways: the shared [`override_item_span`](../../crates/macros/cgp-macro-core/src/functions/override_span.rs) helper re-spans only an item's two *boundary* tokens — its leading `impl` keyword and trailing `{ … }` body — onto the entry, which suffices because the compiler derives the item's span (and the caret) by joining its first and last tokens; a carried span field threads the originating token's span through the evaluated form when the token is synthesized and has lost its span; and reusing the user's own `for` token, as the provider macros do, keeps the middle of a generated impl header off `call_site`. The boundary-only scope is deliberate, not lazy: it leaves a per-entry generic's `<T>` untouched so its `E0207` still points there, and — critically for the IDE — leaves every synthesized *reference* (`IsProviderFor`, `DelegateComponent`) at `call_site` rather than on the entry's range. That matters because rust-analyzer maps a source token to its expanded counterpart purely by source range, ignoring hygiene; a synthesized reference re-spanned onto the key would share the key's range, and go-to-definition on the key would then offer every such construct. Only a keyword and a delimiter group move, never a reference. (`override_item_span` moves a boundary token only when it is itself synthesized, told apart from a user token by source text — a `call_site` token's source text is the whole invocation, which no narrower user token can equal.) Its sibling `override_span` re-spans *every* token unconditionally, which is what `check_components!` needs when it deliberately clobbers one shared user token — the context type — onto each checked component. These spans are testable — a `trybuild` `.stderr` fixture pins the exact line and column of each caret, so a span regression changes the snapshot. The per-macro detail lives in each document's error-spans discussion; [`delegate_components!`](entrypoints/delegate_components.md) and [`#[cgp_impl]`](entrypoints/cgp_impl.md) are the worked examples.
