...
- **`ReplaceSelfValueVisitor`** rewrites every `self` *value* expression to the context identifier, again descending into macro bodies at the token level, and stops at nested `fn` items since they do not capture the outer `self`. Inside a macro body it rewrites only the value form: a `self::` module path is left intact, since a `self` immediately followed by `::` is the current module rather than the receiver.
