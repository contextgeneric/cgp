...
Its parser is where the `.`-versus-`::` separation is decided. It first rejects a leading `@` with a migration hint — the `@Context.` prefix form was replaced by the `in Context` suffix — then parses the trait path, consumes a `.`, and reads either a single `UseTypeIdent` or a brace-delimited comma list of them (`HasTypes.{A, B as C}`). Finally it reads an optional `in Context` suffix, parsing `Context` as a `PathWithTypeArgs`; with no suffix, the context defaults to `Self`. The `.` — not `::` — is what separates the trait from the associated type, so a trait that is itself a path keeps its `::` inside `trait_path` while the `.` unambiguously marks where the associated type begins; the `in` keyword, reserved in Rust, marks the context clause just as cleanly since it can never appear inside a type or path:
...
**A leading `@` is rejected with a migration hint.** The foreign context was once written as an `@Context.` prefix; it is now a trailing `in Context`. `UseTypeAttribute::parse` peeks for a leading `@` before parsing the trait path and returns a spanned error naming the replacement form, rather than letting the `@` fail obscurely inside path parsing. This keeps the old syntax from silently mis-parsing and points the author at the current form.
...
The rejection cases are pinned in [parser_rejections/use_type.rs](../../../../crates/tests/cgp-macro-tests/tests/parser_rejections/use_type.rs): a `=` equality on a component, a duplicate identifier or alias across specs, within one braced list, and on a component, and the removed `@Context.` prefix form.
