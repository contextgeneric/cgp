...
Four acceptable compile-fail fixtures pin the post-codegen failures the attribute defers to the compiler:
...
- [acceptable/cgp_fn/use_type_cyclic_context.rs](../../../../crates/tests/cgp-compile-fail-tests/tests/acceptable/cgp_fn/use_type_cyclic_context.rs) (two `in Context` clauses that reference each other, so grounding never resolves either context and leaves the bare aliases in type position) surfaces as `E0425` "cannot find type" with the caret on the unresolved `in` alias — the sibling of the unresolved-imported-type class, but on the *context* rather than the associated-type name.
