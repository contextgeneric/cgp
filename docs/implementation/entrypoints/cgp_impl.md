...
The one wholly-synthesized item is each `#[default_impl]` delegation impl, built entirely from quasi-quoted tokens whose `impl`/`for` keywords carry `call_site`. `DefaultImplAttribute::to_item_impl` re-spans those synthesized tokens onto the user-written key token — with [`override_synthesized_span`](delegate_components.md#error-spans), the same helper the `delegate_components!` impls use — so a conflict between two default impls for the same key points at the key inside `#[default_impl(Key in …)]` rather than at the whole `#[cgp_impl]` attribute. The user's own tokens keep their spans, so the provider type and any per-entry generic stay navigable in an IDE; nothing needs restoring afterward.
