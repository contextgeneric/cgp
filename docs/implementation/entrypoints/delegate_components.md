...
`build_delegate_component_impl` and `build_is_provider_for_impl` fix this through `respan_impl`, which re-spans only the impl's *macro-synthesized* tokens onto the entry's diagnostic span, leaving the tokens the user wrote at their own spans. The synthesized tokens are the scaffolding — the `impl` keyword, the trait reference, the reserved `__Context__`/`__Params__` generics — and moving them onto the entry moves the header, and therefore the `E0119` caret, onto what the user wrote. The tokens left untouched are the interpolated ones: the key, the wired provider, the target type, and a per-entry generic.

Leaving the user's tokens in place serves two ends at once. A diagnostic about a per-entry generic still points where the user declared it — an unconstrained parameter's `E0207` lands on the `<T>` the user wrote rather than being dragged onto the key — because that `<T>` is a user token the re-span never touches, so no separate step to restore the generics is needed. And a type written in the block stays navigable: an editor resolves go-to-definition on a wired provider by matching the user's token to the same-spanned token in the expansion, so a wired provider resolves to its own definition instead of the entry. (An earlier version re-spanned *every* token of the impl and restored the generics afterward; that clobbered the provider and target-type spans and broke go-to-definition, which is why the re-span is now scoped to synthesized tokens.) The [`override_synthesized_span`](../../../crates/macros/cgp-macro-core/src/functions/override_span.rs) helper it uses recognizes a synthesized token by its `call_site` span. `check_components!` aims its own error the opposite way, with the unconditional `override_span` helper in the same file, because there the intent *is* to clobber a user token — one shared context type, forced onto each checked component in turn.
