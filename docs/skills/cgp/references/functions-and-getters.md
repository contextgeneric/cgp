...
This adds `Self: RectangleArea` to the generated impl's `where` clause, alongside the `HasField` bound from the implicit `scale_factor` — the imported bound lands on the impl only, never on the trait, exactly like writing `where Self: RectangleArea` by hand. The syntax accepts only the simplified `TraitIdent<Params>` form, deliberately: because it is meant to read like an import, it does not accept associated-type-equality bounds such as `Iterator<Item = u8>` — write those as an explicit `where` clause in the body instead. When a provider imports several capabilities, list them all in one attribute separated by commas — `#[uses(CanQueryUserBalance, CanRaiseHttpError<ErrNotFound, String>)]` — rather than stacking `#[uses(...)]` repeatedly; the combined form reads as a single dependency list. `#[uses(...)]` works in both `#[cgp_fn]` and `#[cgp_impl]`, and the imported capability may itself be defined either way.
