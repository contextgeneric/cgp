...
### The theory that ties modules to CGP: modular type classes

The result that makes the CGP comparison precise is that type classes *are* ML modules plus implicit resolution, established by Dreyer, Harper, and Chakravarty. Their *Modular Type Classes* treats **classes as signatures, instances as structures and functors**, and adds a notion of designating certain instance modules as *canonical* within a scope so the compiler can resolve them implicitly ([Dreyer, Harper & Chakravarty, *Modular Type Classes*](https://people.mpi-sws.org/~dreyer/papers/mtc/main-long.pdf)). This is the exact mapping CGP reuses — a component is a signature, a provider is a structure or functor — and it also names the tension CGP resolves: *canonicity* (one instance per type, the coherence property) conflicts with modular abstraction, so modular implicits must limit one to keep the other. CGP simply abandons canonicity, as [bypassing coherence](../concepts/coherence.md) describes, and selects per context instead.
